...
class RXArm(InterbotixManipulatorXS):
    ...
    def __init__(self):
        ...
        self.effort_fb = None
        # DH Params
        self.dh_params = parse_dh_param_file(os.path.join(os.path.dirname(__file__), '../config/rx200_dh_test.csv'))
        # POX params
        ...
        self.destack_des = [[100, -25, 0], [100, -75, 0], [-100, -25, 0], [-100, -75, 0], [-150, -25, 0], [-150, -75, 0], [200, -25, 0], [200, -75, 0]]
    ...
    def set_positions(self, joint_positions, block=False):
        """!
    ...
    def test_inv_kin(self):
        pose = [1,97,54, 2.2]
        target_orient = 0
        IK_geometric(self.dh_params,pose, target_orient)

    # Task 3.3 TODO: Implement click to grab / click to place
    def click_to_grab(self):
        # Perform IK, find joint angles, then grab
        #SAFET = np.array([self.target_pos[0],self.target_pos[1], 290, 3.14])
        #SAFET = np.append(self.target_pos, 2)
        for psi in [np.pi, np.pi*2/3]: #
            BLOCKT = np.append(self.target_pos, psi)
            if psi == np.pi:
                BLOCKT[2] -= 15
            if psi == np.pi*2/3:
                BLOCKT[2] += 20
            init_positions = self.get_positions()
            isReachable, joint_positions = IK_geometric(self.dh_params, BLOCKT, self.target_orient)
            if not isReachable:
                continue
            init_positions[0] = joint_positions[0]
            self.set_positions(init_positions, True)
            BLOCKT[2] += 60
            isReachable, sstage_positions = IK_geometric(self.dh_params, BLOCKT, self.target_orient)
            self.set_positions(sstage_positions, True)
            self.set_positions(joint_positions, True)
            self.gripper.release()
            self.gripper.grasp()
            time.sleep(1)
            return 

    def click_to_place(self, psi = np.pi):
        # Perform IK, find joint angles, then place
        posT = np.append(self.target_pos, psi)
        if psi == np.pi:
            posT[2] += 25
        isReachable, joint_positions = IK_geometric(self.dh_params, posT, self.target_orient)
        if not isReachable:
            return
        posT[2] += 60
        isReachable, sec_positions = IK_geometric(self.dh_params, posT, self.target_orient)
        init_positions = self.get_positions()
        up_T = self.get_ee_pose()
        up_T = np.append(up_T[:3], psi)
        up_T[2] += 150
        print(up_T)
        isReachable, up_positions = IK_geometric(self.dh_params, up_T, self.target_orient)
        init_positions[1] = 0
        init_positions[2] = -np.pi/2
        init_positions[3] = 0
        self.set_positions(up_positions, True)
        self.set_positions(init_positions, True)
        #self.set_positions(sec_positions, True)
        self.set_positions(joint_positions, True)
        self.gripper.release()
        self.set_positions(sec_positions, True)
        self.set_positions([0,0,-np.pi/2,0,0], True)
        return
